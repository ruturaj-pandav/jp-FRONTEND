...
export default function AvailableJobs({ postings }) {
  ...
  return (
    <div>
      <span className="block text-start capitlalize  my-3 text-3xl font-semibold ">Available jobs</span>
      <div className="grid grid-cols-3 gap-4">
        {postings.map((posting, index) => {
          return (
            <div className="p-3 border rounded  shadow-lg">
                <div className="text-start text-xl "> 
                    <span className="block capitalize text-xl font-bold ">{posting.companyName}</span>
                </div>
